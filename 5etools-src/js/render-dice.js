...
Renderer.dice = class {
	...
	static async _pInit () {
		...
		style.innerHTML = `
			...
			.crit-success-label { color: #28a745; font-weight: bold; animation: crit-pulse 0.5s ease; display: inline-block; }
			.crit-fail-label { color: #dc3545; font-weight: bold; animation: crit-shake 0.3s ease; display: inline-block; }
			.crit-icon { margin-right: 4px; font-size: 1.1em; }
			.roll-crit-success { border: 2px solid #28a745; background-color: rgba(40, 167, 69, 0.1); padding: 0 2px; border-radius: 3px; }
			.roll-crit-fail { border: 2px solid #dc3545; background-color: rgba(220, 53, 69, 0.1); padding: 0 2px; border-radius: 3px; }
		`;
	...
	static getEventModifiedRollMeta (evt, entry) {
		...
		let shouldCrit = evt.shiftKey;
		if (Renderer.dice._isNextRollCritical && entry.subType === "damage") {
			shouldCrit = true;
			Renderer.dice._isNextRollCritical = false; // consume the flag
		}
	...
	static _pHandleRoll2_automatic (tree, rolledBy, opts) {
		...
		if (tree) {
			...
			let critLabel = "";
			if (opts.subType === "d20") {
				const critRange = opts.additionalData?.critRange ? Number(opts.additionalData.critRange) : 20;
				// allMax and allMin check if the raw die roll was maximum (20) or minimum (1)
				if (allMax || (meta.diceBaseResult && meta.diceBaseResult >= critRange)) {
					critClass = "roll-crit-success";
					critLabel = `<div style="font-size:0.8rem; color:#81c784; font-weight:bold; margin-top:2px; text-align:center;"><i class="fa-regular fa-star"></i> CRITICAL HIT!</div>`;
					Renderer.dice._isNextRollCritical = true;
				} else if (allMin) {
					critClass = "roll-crit-fail";
					critLabel = `<div style="font-size:0.8rem; color:#e57373; font-weight:bold; margin-top:2px; text-align:center;"><i class="fa-solid fa-triangle-exclamation"></i> CRITICAL MISS!</div>`;
					Renderer.dice._isNextRollCritical = false;
				} else {
					Renderer.dice._isNextRollCritical = false;
				}
			}
			...
			const totalPart = tree.successThresh != null
				? `<span class="roll ${isThreshSuccess && isColorSuccess ? "roll-max" : !isThreshSuccess && isColorFail ? "roll-min" : ""} ${critClass}">${isThreshSuccess ? Renderer.get().render(tree.chanceSuccessText || "Success!") : Renderer.get().render(tree.chanceFailureText || "Failure")}</span>`
				: `<span class="roll ${allMax ? "roll-max" : allMin ? "roll-min" : ""} ${critClass}">${result}</span>`;
...
Renderer.dice.lang = class {
	...
	static getTree3 (str, isSilent = true) {
		...
		if (isSilent) {
			try {
				...
				console.error("getTree3 Error for", str, e);
				return null;
			}
		} else {
			const {lexed, lexedMeta} = Renderer.dice.lang._lex3(str);
	...
	static _lex3_outputToken (self) {
		...
		switch (self.token) {
			...
			case "kh":
			case "k": self.tokenStack.push(Renderer.dice.tk.KEEP_HIGHEST); break;
			...
			case "ro": self.tokenStack.push(Renderer.dice.tk.REROLL_ONCE_EXACT); break;
			case "ro>": self.tokenStack.push(Renderer.dice.tk.REROLL_ONCE_GT); break;
			case "ro>=": self.tokenStack.push(Renderer.dice.tk.REROLL_ONCE_GTEQ); break;
			case "ro<": self.tokenStack.push(Renderer.dice.tk.REROLL_ONCE_LT); break;
			case "ro<=": self.tokenStack.push(Renderer.dice.tk.REROLL_ONCE_LTEQ); break;
			case "x":
			case "!": self.tokenStack.push(Renderer.dice.tk.EXPLODE_EXACT); break;
			case "x>":
			case "!>": self.tokenStack.push(Renderer.dice.tk.EXPLODE_GT); break;
			case "x>=":
			case "!>=": self.tokenStack.push(Renderer.dice.tk.EXPLODE_GTEQ); break;
			case "x<":
			case "!<": self.tokenStack.push(Renderer.dice.tk.EXPLODE_LT); break;
			case "x<=":
			case "!<=": self.tokenStack.push(Renderer.dice.tk.EXPLODE_LTEQ); break;
			case "!!": self.tokenStack.push(Renderer.dice.tk.COMP_EXPLODE_EXACT); break;
			case "!!>": self.tokenStack.push(Renderer.dice.tk.COMP_EXPLODE_GT); break;
			case "!!>=": self.tokenStack.push(Renderer.dice.tk.COMP_EXPLODE_GTEQ); break;
			case "!!<": self.tokenStack.push(Renderer.dice.tk.COMP_EXPLODE_LT); break;
			case "!!<=": self.tokenStack.push(Renderer.dice.tk.COMP_EXPLODE_LTEQ); break;
			case "!p": self.tokenStack.push(Renderer.dice.tk.PEN_EXPLODE_EXACT); break;
			case "!p>": self.tokenStack.push(Renderer.dice.tk.PEN_EXPLODE_GT); break;
			case "!p>=": self.tokenStack.push(Renderer.dice.tk.PEN_EXPLODE_GTEQ); break;
			case "!p<": self.tokenStack.push(Renderer.dice.tk.PEN_EXPLODE_LT); break;
			case "!p<=": self.tokenStack.push(Renderer.dice.tk.PEN_EXPLODE_LTEQ); break;
			case "cs": self.tokenStack.push(Renderer.dice.tk.CRIT_SUCCESS_EXACT); break;
			case "cs>": self.tokenStack.push(Renderer.dice.tk.CRIT_SUCCESS_GT); break;
			case "cs>=": self.tokenStack.push(Renderer.dice.tk.CRIT_SUCCESS_GTEQ); break;
			case "cs<": self.tokenStack.push(Renderer.dice.tk.CRIT_SUCCESS_LT); break;
			case "cs<=": self.tokenStack.push(Renderer.dice.tk.CRIT_SUCCESS_LTEQ); break;
			case "cf": self.tokenStack.push(Renderer.dice.tk.CRIT_FUMBLE_EXACT); break;
			case "cf>": self.tokenStack.push(Renderer.dice.tk.CRIT_FUMBLE_GT); break;
			case "cf>=": self.tokenStack.push(Renderer.dice.tk.CRIT_FUMBLE_GTEQ); break;
			case "cf<": self.tokenStack.push(Renderer.dice.tk.CRIT_FUMBLE_LT); break;
			case "cf<=": self.tokenStack.push(Renderer.dice.tk.CRIT_FUMBLE_LTEQ); break;
			case "cs=":
			case "ts=":
			case "=": self.tokenStack.push(Renderer.dice.tk.COUNT_SUCCESS_EXACT); break;
			case "ts>":
			case ">": self.tokenStack.push(Renderer.dice.tk.COUNT_SUCCESS_GT); break;
			case "ts>=":
			case ">=": self.tokenStack.push(Renderer.dice.tk.COUNT_SUCCESS_GTEQ); break;
			case "ts<":
			case "<": self.tokenStack.push(Renderer.dice.tk.COUNT_SUCCESS_LT); break;
			case "ts<=":
			case "<=": self.tokenStack.push(Renderer.dice.tk.COUNT_SUCCESS_LTEQ); break;
	...
	static _parse3__dice_modifiers (self, children) { // used in both dice and dice pools
		...
		while (
			...
			|| this._parse3_match(self, Renderer.dice.tk.COMP_EXPLODE_EXACT)
			|| this._parse3_match(self, Renderer.dice.tk.COMP_EXPLODE_GT)
			|| this._parse3_match(self, Renderer.dice.tk.COMP_EXPLODE_GTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.COMP_EXPLODE_LT)
			|| this._parse3_match(self, Renderer.dice.tk.COMP_EXPLODE_LTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.PEN_EXPLODE_EXACT)
			|| this._parse3_match(self, Renderer.dice.tk.PEN_EXPLODE_GT)
			|| this._parse3_match(self, Renderer.dice.tk.PEN_EXPLODE_GTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.PEN_EXPLODE_LT)
			|| this._parse3_match(self, Renderer.dice.tk.PEN_EXPLODE_LTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.REROLL_ONCE_EXACT)
			|| this._parse3_match(self, Renderer.dice.tk.REROLL_ONCE_GT)
			|| this._parse3_match(self, Renderer.dice.tk.REROLL_ONCE_GTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.REROLL_ONCE_LT)
			|| this._parse3_match(self, Renderer.dice.tk.REROLL_ONCE_LTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_SUCCESS_EXACT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_SUCCESS_GT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_SUCCESS_GTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_SUCCESS_LT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_SUCCESS_LTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_FUMBLE_EXACT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_FUMBLE_GT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_FUMBLE_GTEQ)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_FUMBLE_LT)
			|| this._parse3_match(self, Renderer.dice.tk.CRIT_FUMBLE_LTEQ)
...
Renderer.dice.tk.COMP_EXPLODE_EXACT = Renderer.dice.tk._new("COMP_EXPLODE_EXACT", "!!", {isDiceModifier: true});
Renderer.dice.tk.COMP_EXPLODE_GT = Renderer.dice.tk._new("COMP_EXPLODE_GT", "!!>", {isDiceModifier: true});
Renderer.dice.tk.COMP_EXPLODE_GTEQ = Renderer.dice.tk._new("COMP_EXPLODE_GTEQ", "!!>=", {isDiceModifier: true});
Renderer.dice.tk.COMP_EXPLODE_LT = Renderer.dice.tk._new("COMP_EXPLODE_LT", "!!<", {isDiceModifier: true});
Renderer.dice.tk.COMP_EXPLODE_LTEQ = Renderer.dice.tk._new("COMP_EXPLODE_LTEQ", "!!<=", {isDiceModifier: true});
Renderer.dice.tk.PEN_EXPLODE_EXACT = Renderer.dice.tk._new("PEN_EXPLODE_EXACT", "!p", {isDiceModifier: true});
Renderer.dice.tk.PEN_EXPLODE_GT = Renderer.dice.tk._new("PEN_EXPLODE_GT", "!p>", {isDiceModifier: true});
Renderer.dice.tk.PEN_EXPLODE_GTEQ = Renderer.dice.tk._new("PEN_EXPLODE_GTEQ", "!p>=", {isDiceModifier: true});
Renderer.dice.tk.PEN_EXPLODE_LT = Renderer.dice.tk._new("PEN_EXPLODE_LT", "!p<", {isDiceModifier: true});
Renderer.dice.tk.PEN_EXPLODE_LTEQ = Renderer.dice.tk._new("PEN_EXPLODE_LTEQ", "!p<=", {isDiceModifier: true});
Renderer.dice.tk.REROLL_ONCE_EXACT = Renderer.dice.tk._new("REROLL_ONCE_EXACT", "ro", {isDiceModifier: true});
Renderer.dice.tk.REROLL_ONCE_GT = Renderer.dice.tk._new("REROLL_ONCE_GT", "ro>", {isDiceModifier: true});
Renderer.dice.tk.REROLL_ONCE_GTEQ = Renderer.dice.tk._new("REROLL_ONCE_GTEQ", "ro>=", {isDiceModifier: true});
Renderer.dice.tk.REROLL_ONCE_LT = Renderer.dice.tk._new("REROLL_ONCE_LT", "ro<", {isDiceModifier: true});
Renderer.dice.tk.REROLL_ONCE_LTEQ = Renderer.dice.tk._new("REROLL_ONCE_LTEQ", "ro<=", {isDiceModifier: true});
Renderer.dice.tk.CRIT_SUCCESS_EXACT = Renderer.dice.tk._new("CRIT_SUCCESS_EXACT", "cs", {isDiceModifier: true});
Renderer.dice.tk.CRIT_SUCCESS_GT = Renderer.dice.tk._new("CRIT_SUCCESS_GT", "cs>", {isDiceModifier: true});
Renderer.dice.tk.CRIT_SUCCESS_GTEQ = Renderer.dice.tk._new("CRIT_SUCCESS_GTEQ", "cs>=", {isDiceModifier: true});
Renderer.dice.tk.CRIT_SUCCESS_LT = Renderer.dice.tk._new("CRIT_SUCCESS_LT", "cs<", {isDiceModifier: true});
Renderer.dice.tk.CRIT_SUCCESS_LTEQ = Renderer.dice.tk._new("CRIT_SUCCESS_LTEQ", "cs<=", {isDiceModifier: true});
Renderer.dice.tk.CRIT_FUMBLE_EXACT = Renderer.dice.tk._new("CRIT_FUMBLE_EXACT", "cf", {isDiceModifier: true});
Renderer.dice.tk.CRIT_FUMBLE_GT = Renderer.dice.tk._new("CRIT_FUMBLE_GT", "cf>", {isDiceModifier: true});
Renderer.dice.tk.CRIT_FUMBLE_GTEQ = Renderer.dice.tk._new("CRIT_FUMBLE_GTEQ", "cf>=", {isDiceModifier: true});
Renderer.dice.tk.CRIT_FUMBLE_LT = Renderer.dice.tk._new("CRIT_FUMBLE_LT", "cf<", {isDiceModifier: true});
Renderer.dice.tk.CRIT_FUMBLE_LTEQ = Renderer.dice.tk._new("CRIT_FUMBLE_LTEQ", "cf<=", {isDiceModifier: true});
Renderer.dice.tk.COUNT_SUCCESS_EXACT = Renderer.dice.tk._new("COUNT_SUCCESS_EXACT", "=", {isDiceModifier: true, isSuccessMode: true});
Renderer.dice.tk.COUNT_SUCCESS_GT = Renderer.dice.tk._new("COUNT_SUCCESS_GT", ">", {isDiceModifier: true, isSuccessMode: true});
Renderer.dice.tk.COUNT_SUCCESS_GTEQ = Renderer.dice.tk._new("COUNT_SUCCESS_GTEQ", ">=", {isDiceModifier: true, isSuccessMode: true});
Renderer.dice.tk.COUNT_SUCCESS_LT = Renderer.dice.tk._new("COUNT_SUCCESS_LT", "<", {isDiceModifier: true, isSuccessMode: true});
Renderer.dice.tk.COUNT_SUCCESS_LTEQ = Renderer.dice.tk._new("COUNT_SUCCESS_LTEQ", "<=", {isDiceModifier: true, isSuccessMode: true});
...
Renderer.dice.parsed = class {
	...
	static _handleModifiers (fnName, meta, vals, nodeMod, opts) {
		...
		for (const mod of mods) {
			...
			let modNum = 1;
			if (mod.numSym) {
				modNum = mod.numSym[fnName]();
			} else {
				// Default behavior for implicit modifiers
				const type = mod.modSym.type;
				if (type.includes("EXPLODE") || type.includes("CRIT_SUCCESS") || type.includes("MARGIN_SUCCESS")) {
					modNum = faces; // Explodes or crits default to maximum face value
				} else if (type.includes("CRIT_FUMBLE")) {
					modNum = 1; // Fumbles default to 1
				} else {
					modNum = 1; // Rerolls default to 1
				}
			}
			...
			switch (mod.modSym.type) {
				...
				case Renderer.dice.tk.REROLL_ONCE_EXACT.type:
				case Renderer.dice.tk.REROLL_ONCE_GT.type:
				case Renderer.dice.tk.REROLL_ONCE_GTEQ.type:
				case Renderer.dice.tk.REROLL_ONCE_LT.type:
				case Renderer.dice.tk.REROLL_ONCE_LTEQ.type: {
					let fnPartition;
					let isOnce = false;
					switch (mod.modSym.type) {
						...
						case Renderer.dice.tk.REROLL_ONCE_EXACT.type: fnPartition = Renderer.dice.parsed._PARTITION_EQ; isOnce = true; break;
						case Renderer.dice.tk.REROLL_ONCE_GT.type: fnPartition = Renderer.dice.parsed._PARTITION_GT; isOnce = true; break;
						case Renderer.dice.tk.REROLL_ONCE_GTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_GTEQ; isOnce = true; break;
						case Renderer.dice.tk.REROLL_ONCE_LT.type: fnPartition = Renderer.dice.parsed._PARTITION_LT; isOnce = true; break;
						case Renderer.dice.tk.REROLL_ONCE_LTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_LTEQ; isOnce = true; break;
						default: throw new Error(`Unimplemented!`);
					}

					let tries = 999;
					let toRerollNext = valsAlive.filter(val => fnPartition(val.val, modNum));
					
					while (toRerollNext.length && tries-- > 0) {
						toRerollNext.forEach(val => val.isDropped = true);

						const nuVals = opts.fnGetRerolls(toRerollNext);

						vals.push(...nuVals);
						displayVals.push(...nuVals);

						if (isOnce) break;
						toRerollNext = nuVals.filter(val => fnPartition(val.val, modNum));
					}
					break;
				}
				...
				case Renderer.dice.tk.COMP_EXPLODE_EXACT.type:
				case Renderer.dice.tk.COMP_EXPLODE_GT.type:
				case Renderer.dice.tk.COMP_EXPLODE_GTEQ.type:
				case Renderer.dice.tk.COMP_EXPLODE_LT.type:
				case Renderer.dice.tk.COMP_EXPLODE_LTEQ.type:
				case Renderer.dice.tk.PEN_EXPLODE_EXACT.type:
				case Renderer.dice.tk.PEN_EXPLODE_GT.type:
				case Renderer.dice.tk.PEN_EXPLODE_GTEQ.type:
				case Renderer.dice.tk.PEN_EXPLODE_LT.type:
				case Renderer.dice.tk.PEN_EXPLODE_LTEQ.type: {
					let fnPartition;
					let isComp = false;
					let isPen = false;
					switch (mod.modSym.type) {
						...
						case Renderer.dice.tk.COMP_EXPLODE_EXACT.type: fnPartition = Renderer.dice.parsed._PARTITION_EQ; isComp = true; break;
						case Renderer.dice.tk.COMP_EXPLODE_GT.type: fnPartition = Renderer.dice.parsed._PARTITION_GT; isComp = true; break;
						case Renderer.dice.tk.COMP_EXPLODE_GTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_GTEQ; isComp = true; break;
						case Renderer.dice.tk.COMP_EXPLODE_LT.type: fnPartition = Renderer.dice.parsed._PARTITION_LT; isComp = true; break;
						case Renderer.dice.tk.COMP_EXPLODE_LTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_LTEQ; isComp = true; break;
						case Renderer.dice.tk.PEN_EXPLODE_EXACT.type: fnPartition = Renderer.dice.parsed._PARTITION_EQ; isPen = true; break;
						case Renderer.dice.tk.PEN_EXPLODE_GT.type: fnPartition = Renderer.dice.parsed._PARTITION_GT; isPen = true; break;
						case Renderer.dice.tk.PEN_EXPLODE_GTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_GTEQ; isPen = true; break;
						case Renderer.dice.tk.PEN_EXPLODE_LT.type: fnPartition = Renderer.dice.parsed._PARTITION_LT; isPen = true; break;
						case Renderer.dice.tk.PEN_EXPLODE_LTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_LTEQ; isPen = true; break;
						default: throw new Error(`Unimplemented!`);
					}
					...
					do {
						...
						const [toExplode] = toExplodeNext.partition(roll => !roll.isExploded && fnPartition(roll.originalVal || roll.val, modNum));
						toExplode.forEach(roll => roll.isExploded = true);

						const nuVals = opts.fnGetExplosions(toExplode);
						if (isPen) nuVals.forEach(v => v.val = Math.max(0, v.val - 1));

						toExplodeNext = nuVals;

						if (isComp) {
							toExplode.forEach((parentRoll, i) => {
								let root = parentRoll;
								while (root.isCompParent) root = root.isCompParent;
								root.originalVal = root.originalVal || root.val;
								root.val += nuVals[i].val;
								nuVals[i].isHidden = true;
								nuVals[i].isCompParent = root;
							});
						}

						vals.push(...nuVals);
						if (!isComp) displayVals.push(...nuVals);
				...
				case Renderer.dice.tk.CRIT_SUCCESS_EXACT.type:
				case Renderer.dice.tk.CRIT_SUCCESS_GT.type:
				case Renderer.dice.tk.CRIT_SUCCESS_GTEQ.type:
				case Renderer.dice.tk.CRIT_SUCCESS_LT.type:
				case Renderer.dice.tk.CRIT_SUCCESS_LTEQ.type:
				case Renderer.dice.tk.CRIT_FUMBLE_EXACT.type:
				case Renderer.dice.tk.CRIT_FUMBLE_GT.type:
				case Renderer.dice.tk.CRIT_FUMBLE_GTEQ.type:
				case Renderer.dice.tk.CRIT_FUMBLE_LT.type:
				case Renderer.dice.tk.CRIT_FUMBLE_LTEQ.type: {
					let fnPartition;
					let isFumble = false;
					switch (mod.modSym.type) {
						case Renderer.dice.tk.CRIT_SUCCESS_EXACT.type: fnPartition = Renderer.dice.parsed._PARTITION_EQ; break;
						case Renderer.dice.tk.CRIT_SUCCESS_GT.type: fnPartition = Renderer.dice.parsed._PARTITION_GT; break;
						case Renderer.dice.tk.CRIT_SUCCESS_GTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_GTEQ; break;
						case Renderer.dice.tk.CRIT_SUCCESS_LT.type: fnPartition = Renderer.dice.parsed._PARTITION_LT; break;
						case Renderer.dice.tk.CRIT_SUCCESS_LTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_LTEQ; break;
						case Renderer.dice.tk.CRIT_FUMBLE_EXACT.type: fnPartition = Renderer.dice.parsed._PARTITION_EQ; isFumble = true; break;
						case Renderer.dice.tk.CRIT_FUMBLE_GT.type: fnPartition = Renderer.dice.parsed._PARTITION_GT; isFumble = true; break;
						case Renderer.dice.tk.CRIT_FUMBLE_GTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_GTEQ; isFumble = true; break;
						case Renderer.dice.tk.CRIT_FUMBLE_LT.type: fnPartition = Renderer.dice.parsed._PARTITION_LT; isFumble = true; break;
						case Renderer.dice.tk.CRIT_FUMBLE_LTEQ.type: fnPartition = Renderer.dice.parsed._PARTITION_LTEQ; isFumble = true; break;
						default: throw new Error(`Unimplemented!`);
					}

					const crits = valsAlive.filter(val => fnPartition(val.originalVal || val.val, modNum));
					if (isFumble) {
						crits.forEach(val => val.isCritFumble = true);
					} else {
						crits.forEach(val => val.isCritSuccess = true);
					}
					break;
				}
	...
	static Pool = class extends Renderer.dice.AbstractSymbol {
		...
		_invoke (fnName, meta) {
			...
			if (this._nodeMod && vals.length) {
				...
				if (isSuccessMode) {
					return vals.filter(it => !it.isDropped && !it.isHidden && it.isSuccess).length;
				} else {
					return vals.filter(it => !it.isDropped && !it.isHidden).map(it => it.val).sum();
				}
			} else {
				this.addToMeta(
					meta,
					["html", "text", "md"].mergeMap(prop => ({
						[prop]: `${vals.filter(it => !it.isHidden).map(it => `(${it.meta[prop].join("")})`).join("+")}`,
					})),
				);
				return vals.filter(it => !it.isHidden).map(it => it.val).sum();
			}
		}
	...
	static Dice = class extends Renderer.dice.AbstractSymbol {
		...
		_invoke (fnName, meta) {
			...
			while (view.length) {
				...
				const isLast = view.length === 0 || (view.length === 1 && view[view.length - 1].isDiceModifierGroup);
		...
		_invoke_handlePart (fnName, meta, view, num, faces, isLast) {
			...
			if (isLast) { // only display the dice for the final roll, e.g. in 2d3d4 show the Xd4
				if (fnName === "evl") {
					meta.diceList = meta.diceList || [];
					displayRolls.forEach(r => {
						if (typeof r.val === "number") {
							meta.diceList.push({ faces, val: r.val, isDropped: r.isDropped });
						}
					});
				}
			...
			if (isSuccessMode) {
				return rolls.filter(it => !it.isDropped && !it.isHidden && it.isSuccess).length;
			} else {
				return rolls.filter(it => !it.isDropped && !it.isHidden).map(it => it.val).sum();
			}
		}
