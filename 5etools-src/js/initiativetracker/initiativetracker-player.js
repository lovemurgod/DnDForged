...
export class InitiativeTrackerPlayerMessageHandlerV1 {
	...
	_handleMessage_state ({payload}) {
		...
		if (data.round) {
			...
			if (roundNum) roundNum.textContent = data.round;
		}
		...
		// Ensure the active element is scrolled into view in the horizontal carousel
		setTimeout(() => {
			const activeEl = this._eleRows[0].querySelector('.initp__r--active');
			if (activeEl) {
	...
	_getRow (rowData) {
		...
		const hpPillHtml = (hpText && hpText !== "?" && hpText !== "?/?")
			? `<div class="initp__r_hp_pill" style="background: ${hpColor};">${hpText}</div>`
			: "";

		return ee`
				<div class="initp__r ${rowData.isActive ? `initp__r--active` : ""}">
					<div class="ve-w-100 ve-split-v-center ve-min-w-100p" style="flex-direction: column; align-items: center; text-align: center;">
						${this._getRenderedStatsCells({rowData})}
						<div style="font-weight: bold; margin-bottom: 4px;">${dispName}</div>
						${wrpConds}
					</div>
					<div class="ve-min-w-100p" style="width: 100%;">
						${hpPillHtml}
					</div>
					<div class="initp__r_score ${this._isCompact ? "initp__r_score--compact" : ""}">Init: ${rowData.initiative}</div>
				</div>
			`;
	}
